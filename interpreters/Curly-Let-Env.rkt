...
(require (rename-in "Curly-Let.rkt"
                    [interp slow-interp]
                    [run slow-run]
                    [subst old-subst]))
...
(define (interp [env : Env]
  ...
  (type-case Expr e
    ...
    [(Var x)
     (lookup x env)]
    ;; Instead of substituting, we interpret in an extended environment
    [(Call funName argExpr)
     (let* ([argVal (interp env defs argExpr)] ;;Evaluate the argument
       ...
       (interp (extendEnv (bind argVar argVal) emptyEnv) ;; !!
               defs funBody))]
    ;; NEW: Interpret a let by interpreting the body in an extended environment
    [(LetVar x xexp body)
