...
;; Curly-WhileRec
;; This language extends Curly-Mutvar to include two forms of self-reference:
...
;; NEW
;;   | {while <expr> <expr>} ;; Evaluate the second expression as long as the first evaluates to true
;;   | {letrec {SYMBOL <expr>} <expr>} ;; Just like let1, but in {letrec {x e} body},
;;                                         x is allowed to occur in e, enabling self-reference
...
(define-type ExpS
  ...
  ;;NEW
  ;; Runs the body as long as the condition evaluates to true
  (whileS [cond : ExpS]
          [body : ExpS])
  ;; Like let1, but var refers to val inside val, so we have recursion
  (letrecS [var : Symbol]
           [val : ExpS]
           [body : ExpS])
  )
...
(define-type Value
  ...
  [boxV [loc : Location]]
  ;; NEW
  ;; Dummy values that only exist as placeholders for recursion
  ;; If interp ever returns this, then somebody wrote a bad recursive function
  [dummyV]
  )
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;  Environments map symbols to locations, not values
...
(define (parse [s : S-Exp]) : ExpS
  (cond
    ...
    [(s-exp-match? `{while ANY ANY} s)
     (whileS (parse (second (s-exp->list s)))
           (parse (third (s-exp->list s))))]
    ;; Recursive Variable definitions
    ;; {letrec {x val} body}
    [(s-exp-match? `{letrec {SYMBOL ANY} ANY} s)
     (letrecS (s-exp->symbol (first (s-exp->list (second (s-exp->list s)))))
            (parse (second (s-exp->list (second (s-exp->list s)))))
            (parse (third (s-exp->list s))))]
    ;; Function calls
...
(define (desugar [es : ExpS]) : Exp
  (type-case ExpS es
    ...
    [(address-ofS var)
     (address-ofE var)]
    ;; NEW
    ;; Basic desugaring for while and letrec
    [(whileS cond body)
     (whileE (desugar cond) (desugar body))]
    [(letrecS var val body)
     (letrecE var (desugar val) (desugar body))]
    ))
...
(define (interp [env : Env]
  ...
  (type-case Exp e
    ...
    [(appE fun arg)
     ...
     ;; When we call a function, we need to allocate a new store location for its parameter
    ...
    ;; New
    [(whileE cond body)
     ;; Run the condition expression to a Result
     (with [(cond-v cond-sto) (interp env cond sto)]
           ;; Check if the result value is a boolean
           ;; and raise a type error if it's not
           (type-case Value cond-v
             [(boolV b)
              ;; Check if the boolean is true. If it is,
              ;; then run the body, discard the results, and execute the while loop again,
              ;; but in the modified state from running the body
              ;; Otherwise, produce the value #f and don't change the store further
              (if b
                  ;; Loop condition was true
                  (with [(body-v body-sto)  (interp env body cond-sto)]
                        (interp env (whileE cond body) body-sto))
                  ;; Loop condition was false, return dummy value #f
                  ;; And the store with the side-effects from running the condition,
                  ;; but NOT from running the body
...
;; While loop tests
;; False condition should never run loop body
(test (run `{let1 {x 3}
                  {seq {while #f {set-var! x 99}}
                       x}})
      (numV 3))

;; Sum the first 10 numbers using a while loop
(test (run
       `{let1 {sum 0}
              {let1 {i 0}
                    {seq
                       {while {not {= i 11}}
                              {seq {set-var! sum {+ sum i}}
                                   {set-var! i {+ i 1}}}}
                       sum}}})
      (numV 55))
...
(test (run
       `{letrec {fact {lam x {if {= x 0}
                                 1
                                 {* x {fact {- x 1}}}}}}
          {fact 5}})
      (numV 120))

;; If we self-reference not behind a lambda, we should get dummyV,
;; and using it should be an error
(test/exn (run `{letrec {x {+ x 1}}
                  x}) "")