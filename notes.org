...
- Functions are fundamentally about /substitution/:
...
* Glossary 
- Semantics of Let for Single Variables :: For any expression ~EXPR~, we have
~(let ([x EXPR]) body)~

- Semantics of Function Calls :: If a function ~f~ is defined as:
- ~(define (f x) body)~
then for any expression ~EXPR~, we have:
- ~(f EXPR) = [x => EXPR]body~
where ~[x => EXPR]body~ is ~body~, with all non-shadowed occurrences of the variable ~x~ replaced by ~EXPR~
