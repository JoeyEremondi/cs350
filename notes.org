...
*** S-expressions
The syntax of Dr. Racket based around /S-Expressions/. Every S-Expression is either:
- A literal (such as a number ~3~, a boolean ~#t~ or ~#f~, a string ~"hello"~, a symbol ~'a~, etc.)
- A name (such as ~x~, ~y~, ~foo~, ~map~, ~if~, ~type-case~ )
- Parentheses ~(...)~ containing one or more S-Expressions, separated by a space

Since an S-Expression can contain more s-expressions inside of it, they are effectively a notation
...
After the ~#lang~ line, the contents of a Racket (or Flit) file are all S-Expressions.
Some are commands to Racket, like ~(require ...)~ for importing other files or ~(test expected actual)~
for declaring a test.
Some are definitions ~(define someThing ...), which define functions.
...
- ~# | \ , @~ are reserved for other purposes
  ...
  Boolean-returning functions usually have names ending in ~?~.
*** Expressions in Flit
...
****  Literals
Literals are expressions that evaluate to themselves. For example:
#+begin_src racket
3 ;; The number 3
#f ;; False
#t ;; True
...
the expression will compute however the keyword is defined.

The main keywords we'll use are ~if~ and ~type-case~. We'll explain ~type-case~ in a later chapter.
